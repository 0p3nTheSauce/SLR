    ...
    for loc in KEYS:
        ...
        for run in que_list:
            ...
            for key in keys:
                ...
                k = "spatial_aug"
                data[key][k] = [map_frame_size_strat_to_crop_config(fss, fs)] + data[
                    key
                ][k]
                k = "temporal_aug"
...
def test_set_inplace():
    ...
    dicty2 = {"d1": dicty, "d2": rot, "d3": opdicty}
...
def test_edit_run():
    ...
    demo = {
        ...
        "model_params": {"drop_p": 0.5, "type": "supervised"},
        "data": {
            "train_augs": {
                "normalise": True,
                "norm_dict": {"mean": [0.45, 0.45, 0.45], "std": [0.225, 0.225, 0.225]},
                "temporal_aug": [
                    {"target_length": 16, "max_wobble": 4, "type": "chunked"}
                ],
                "spatial_aug": [
                    {"type": "HORIZONTAL_FLIP", "p": 0.5},
                    {"frame_size": 224, "type": "Centre_crop"},
                    {
                        "type": "RANDAUGMENT",
                        "num_ops": 3,
                        "magnitude": 7,
            ...
            "test_augs": {
                ...
                "norm_dict": {"mean": [0.45, 0.45, 0.45], "std": [0.225, 0.225, 0.225]},
                "temporal_aug": [
                    {"target_length": 16, "max_wobble": 0, "type": "uniform"}
                ],
...
def test_make_list_section():
    from src.que.runs_to_configs import _make_list_section
    ...
    section = _make_list_section(
        dummy_section, f"{sec_name}.{subsec_name}.{subsubsec_name}"
    )
    print(section)


def test_make_nested_section():
    from src.que.runs_to_configs import _handle_dict
...
def test_make_full_config():
    from src.que.runs_to_configs import run_to_config
    ...
    config_str = run_to_config(
        run_info, comments=["This is a comment", "Another comment"]
    )
    print(config_str)
...
def test_update_all_files(
    ...
    for run_info in flat_all_runs[-2:-1]:
...
if __name__ == "__main__":
    ...
    debug_path = Path(__file__).resolve().with_suffix('.toml')
    
    
    
    test_update_all_files(
        dry_run=True,
        retro_support=False,
        output=debug_path
        )
