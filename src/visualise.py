...
def plot_model_comparison_bar(
    ...
    ax.set_title(
        f"{dataset.upper()} - {title_type} Accuracy ({split.capitalize()} Set)",
        fontsize=14,
    )
...
def plot_heatmap_comparison(
    df: pd.DataFrame, metric: str = "top1_avg", split: str = "test"
):
    """
    ...
    heatmap_df = (
        df[df["split"] == split].pivot(index="model", columns="dataset", values=metric)
        * 100
    )
    ...
    metric_name = (
        metric.replace("_", " ")
        .title()
        .replace("Avg", "(Avg per Class)")
        .replace("Inst", "(Per Instance)")
    )
...
def plot_top_k_progression(
    ...
    title_type = "Average per Class" if metric_type == "avg" else "Per Instance"
    ax.set_title(
        f"{dataset.upper()} - Top-K {title_type} Accuracy ({split.capitalize()} Set)",
        fontsize=14,
    )
...
def plot_dataset_comparison_scatter(
    df: pd.DataFrame, metric: str = "top1_avg", split: str = "test"
):
    """
    ...
    """
    # Prepare data
    asl100_df = df[(df["dataset"] == "asl100") & (df["split"] == split)][
        ["model", metric]
    ]
    asl300_df = df[(df["dataset"] == "asl300") & (df["split"] == split)][
        ["model", metric]
    ]
    ...
    # Scatter plot
    ax.scatter(
        merged[f"{metric}_100"] * 100, merged[f"{metric}_300"] * 100, s=100, alpha=0.7
    )
    ...
    ax.set_title(
        f"Model Performance: ASL100 vs ASL300 ({split.capitalize()} Set)", fontsize=14
    )
    ax.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    df, results = run_visualizations(
        "./results/wlasl_satnac_16_only_summary.json",
        extractor=extract_metrics_df_sumed,
        splits=["asl100"],
    )
