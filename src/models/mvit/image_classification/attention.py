...
def attention_pool(tensor, pool, hw_shape, has_cls_embed=True, norm=None):
    ...
    if has_cls_embed:
        tensor = torch.cat((cls_tok, tensor), dim=2) #type: ignore
    if norm is not None:
        tensor = norm(tensor)
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        if mode in ("avg", "max"):
            ...
            self.pool_q = (
                pool_op(kernel_q, stride_q, padding_q, ceil_mode=False) #type: ignore
                if len(kernel_q) > 0
                else None
            )
            self.pool_k = (
                pool_op(kernel_kv, stride_kv, padding_kv, ceil_mode=False) #type: ignore
                if len(kernel_kv) > 0
                else None
            )
            self.pool_v = (
            ...
            self.pool_k = (
                nn.Conv2d(
                    ...
                    kernel_kv,
                    stride=stride_kv,
                    padding=padding_kv, #type: ignore
                    groups=dim_conv,
                    bias=False,
                )
                if len(kernel_kv) > 0
    ...
    def forward(self, x, hw_shape):
        ...
        if self.pool_first:
            ...
            q = q.permute(0, 2, 1, 3).reshape(B, q_N, -1) #type: ignore
            q = self.q(q).reshape(B, q_N, self.num_heads, -1).permute(0, 2, 1, 3)

            v = v.permute(0, 2, 1, 3).reshape(B, v_N, -1) #type: ignore
            v = self.v(v).reshape(B, v_N, self.num_heads, -1).permute(0, 2, 1, 3)

            k = k.permute(0, 2, 1, 3).reshape(B, k_N, -1) #type: ignore
    ...
    def __init__(
        ...
        if len(stride_q) > 0 and numpy.prod(stride_q) > 1:
            ...
            self.pool_skip = nn.MaxPool2d(
                kernel_skip, stride_skip, padding_skip, ceil_mode=False #type: ignore
            )
        else:
            self.pool_skip = None
