...
def _setup_wandb(
    ...
    if wandb_info.sweep_id:
        #generated by sweep
        run_name = f"{admin.model}_{admin.split}_{wandb_info.run_id}"
    else:
        #hand made
        run_name = f"{admin.model}_{admin.split}_exp{admin.exp_no}"
        
    #recover run on wandb
    if admin.recover:
        #get run_id
        ...
        if run_id is not None:
            ...
            run = wandb.init(
                ...
                id=run_id,
                resume="must",
                settings=wandb.Settings(sweep_id=wandb_info.sweep_id)
            )
