...
def train_model_semi_contrastive(model, train_loader, optimizer, contr_loss_func, 
                                 super_loss_func, temprerature=0.5,
                                 epochs=10,val_loader=None, scheduler=None,
                  output='runs/exp_0', logs='logs', save='checkpoints', save_every=1, load=None,
                  cutoff_train_loss=0.1):
  device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
  model.to(device)
  ...
  try:
    for epoch in pbar:
      ...
      for batch in train_loader:
        ...
        view1, view2, label = view1.to(device), view2.to(device), label.to(device)
        ...
        z2 = model(view2)
        # loss = loss_func(model_output, target)
        loss_cont = contr_loss_func(z1, z2)
        loss_sup = super_loss_func(z1, label)
        comb_loss = (1 - temprerature) * loss_sup + temprerature * loss_cont
        comb_loss.backward()
        optimizer.step()
        
        #Accumulate metrics
        running_loss += comb_loss.item() * view1.size(0) #weight by batch size
        train_samples += view1.size(0)
        # _, predicted = model_output.max(1)
        # train_correct += predicted.eq(target).sum().item()
      ...
      if val_loader:
        ...
        with torch.no_grad():
          for batch in val_loader:
            ...
            comb_loss = (1 - temprerature) * loss_sup + temprerature * loss_cont  
            #Accumulate validation metrics
            val_loss += comb_loss.item() * view1.size(0) #weight by batch size
            val_samples += view1.size(0)
            # _, predicted = model_output.max(1)
            # val_correct += predicted.eq(target).sum().item()
