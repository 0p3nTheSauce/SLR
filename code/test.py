...
def summarise(
    ...
    splits: list[str],
    model_exps: list[list[tuple[str, str]]],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Test all experiments in runs directory (overrides other inputs)",
    )
    parser.add_argument(
        "--test_last",
        action="store_true",
    ...
    parser.add_argument(
        "--top_k", action="store_true", help="Report top-k accuracies (default True)"
    )
    parser.add_argument(
        "--plot",
        action="store_true",
        help="Create plots (heatmap, bargraph, confusion matrix)",
    )
    parser.add_argument(
        "--disp",
        action="store_true",
        help="Display plots (if plot) (using utils.visualise_frames)",
    )
    parser.add_argument(
        "--res_output",
        type=str,
        default=None,
        help="Path to write results to (default does not save result dict)",
    )
    parser.add_argument(
        "--skip_done",
        action="store_true",
        help="Skips directories that have been tested (if they have result files) (default True)",
    )
    parser.add_argument(
        "--test_val",
        action="store_true",
        help="Test on the validation set as well (default only test set)",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
        help="Shuffle the frames before inference (using Shuffle from video_transforms)",
    )
    parser.add_argument(
        "--imp_path",
        type=str,
        default="wlasl_implemented_info.json",
        help="Path to implemented info (default wlasl_implemented_info.json)",
    )
    parser.add_argument(
        "--classes_path",
        type=str,
        default="wlasl_class_list.json",
        help="Path to class list file (if plot) (default wlasl_class_list.json)",
    )
