...
from typing import List, Dict, TypedDict, TypeAlias, Literal, Optional, Union, Any, Iterable
...
def plot_length_distribution(histogram: HistoGram, gloss: str, set_name: AVAIL_SETS) -> None:
    ...
    plt.bar(lengths, counts, width=2, label="Instances")
    plt.axvline(mean_length, color='red', linestyle='--', label=f"Mean: {mean_length:.1f} frames")
    plt.xlabel("Video Length (frames)")
    plt.ylabel("Count")
    plt.title(f'Length Distribution: "{gloss}" ({set_name})')
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def collect_class_stats(instances: List[instance_dict]) -> class_stats:
    ...
    for instance in instances:
        num_instances += 1
        instance_with_length = {**instance | {"length":instance["frame_end"] - instance['frame_start']}} #add length key
...
def plot_distribution(
    ...
    if categorical:
        ...
        mean_val = sum(v * c for v, c in zip(values, counts)) / sum(counts)
        plt.axvline(mean_val, color='red', linestyle='--', 
                    label=f"Mean: {mean_val:.1f}{' ' + unit if unit else ''}")
        plt.legend()
