...
class MultiScaleAttention(nn.Module):
    ...
    def __init__(
        ...
        self.pool_q = nn.Conv2d(
            ...
            pool_kernel,
            stride=stride_q,
            padding=pool_padding, #type: ignore
            groups=dim_conv,
            bias=False,
        )
        ...
        if self.use_rel_pos:
            ...
            assert input_size[0] == input_size[1] #type: ignore
            size = input_size[0] #type: ignore
    ...
    def forward(self, x):
        ...
        q = q.view(q.shape[0], np.prod(q_hw), -1) #type: ignore
        k = k.view(k.shape[0], np.prod(kv_hw), -1) #type: ignore
        v = v.view(v.shape[0], np.prod(kv_hw), -1) #type: ignore
        ...
        if self.window_size:
            x = window_unpartition(x, self.q_win_size, q_hw_pad, ori_q.shape[1:3]) #type: ignore
