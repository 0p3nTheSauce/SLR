...
class MViTv2S_basic(nn.Module):
	...
	def _adapt_pos_encoding(self, num_frames: int):
		...
		spatial_embed = torch.nn.functional.interpolate(
			spatial_embed, size=(new_temporal_size, H * W), mode="bilinear", align_corners=False
		)
		spatial_embed = spatial_embed.permute(0, 2, 3, 1).reshape(1, new_temporal_size * H * W, C)
