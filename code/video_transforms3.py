...
def sample_uniform(frames: Tensor, target_length: int) -> Tensor:
...
def _apply_wobble_then_sample(
	frames: Tensor, target_length: int, *, fn: Callable, max_wobble: int
) -> Tensor:
	return fn(wobble(frames, max_wobble), target_length)


def _call_sampler(
	frames: Tensor, *, fn: Callable[[Tensor, int], Tensor], target_length: int
) -> Tensor:
...
def get_sampler(sampler_config: SamplerConfig) -> Transform:
	...
	if sampler_config.max_wobble > 0:
		final_sampler = partial(
			_apply_wobble_then_sample, fn=sampler, max_wobble=sampler_config.max_wobble
		)
	else:
	...
	return v2.Lambda(
		partial(
			_call_sampler, fn=final_sampler, target_length=sampler_config.target_length
		)
	)
...
def _interp_sets() -> dict[str, v2.AutoAugmentPolicy]:
	return {
...
def _permute_time_channel(x):
	"""Permute tensor from (T, C, H, W) to (C, T, H, W)"""
...
def get_transform(
	...
	for aug in temporal_aug:
		transforms_list.append(get_temporal_augs(aug))
