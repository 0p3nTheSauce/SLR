...
class queShell(cmdLib.Cmd):
	...
	def do_list(self, arg):
		...
		table.add_column("Run Description", style="white")
		
		# Since runs are strings, display them directly
	...
	def do_attach(self, arg):
		"""Attach to tmux with info panel"""
		parsed_args = self._parse_args_or_cancel("attach", arg)
		if parsed_args is None:
			return

		self.console.print(Panel(
			f"[cyan]Attaching to {parsed_args.window} session...\n"
			"[dim]Press Ctrl+B then D to detach[/dim]",
			border_style="cyan"
		))
		self.tmux_man.join_session(parsed_args.window)

	def do_daemon(self, arg):
		"""Start daemon with status display"""
		...
		with self.console.status("[bold cyan]Starting daemon...", spinner="dots"):
			self.que.save_state()
			self.tmux_man.start(self.dn_name, parsed_args.setting, ext_args=ext_args)
			time.sleep(5)
			self.que.load_state()
		
		self.console.print(f"[bold green]✓[/bold green] Daemon started with setting: {parsed_args.setting}")

	def do_worker(self, arg):
		"""Start worker with status display"""
		parsed_args = self._parse_args_or_cancel("worker", arg)
		if parsed_args is None:
			return

		with self.console.status("[bold cyan]Starting worker...", spinner="dots"):
			self.tmux_man.start(self.wr_name, parsed_args.setting)
		
		self.console.print(f"[bold green]✓[/bold green] Worker started with setting: {parsed_args.setting}")
