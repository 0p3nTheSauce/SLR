...
if [[ "$MODE" == "server" ]]; then
    ...
    # ---------------------------------------------------------------------------
    # start_server - wrapper for que server startup
...
START_SERVER_WRAPPER
    ...
    if [ ! -f "$START_SERVER_SCRIPT" ]; then
        echo -e "${RED}Error: start_server.sh not found at $START_SERVER_SCRIPT${NC}"
        exit 1
    fi
    ...
    echo "  Start Script:      $START_SERVER_SCRIPT"
    echo ""
...
if [[ "$MODE" == "server" ]]; then
    echo "  Service mode:  $SERVICE_NAME"
    echo "  Start script:  $START_SERVER_SCRIPT"
fi
echo ""
