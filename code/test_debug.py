...
def sim_train_script():
	arg_parser = argparse.ArgumentParser(
		description="Simulate a wandb run for waiting testing."
	)
	arg_parser.add_argument(
		"-p", "--project", type=str, default="test_quewing", help="Wandb project name"
	)
	arg_parser.add_argument(
		"-n",
		"--name",
		type=str,
		default="test_wait_for_run_completion",
		help="Wandb run name",
	)
	arg_parser.add_argument(
		"-e",
		"--entity",
		type=str,
		default="ljgoodall2001-rhodes-university",
	...
	arg_parser.add_argument(
		"--early_stop", action="store_true", help="Enable early stopping"
	)
	arg_parser.add_argument(
		"--patience",
		type=int,
	...
	print(
		f"Project: {args.project}, Name: {args.name}, Entity: {args.entity}, Sleep: {args.sleep}, Early Stop: {args.early_stop}"
	)
...
def test_stopper_save_and_load(stopper, arg_dict):
	...
	assert stopper.best_score is not None, (
		"Best score should not be None after loading state"
	)
	print(
		f"Restored best score: {stopper.best_score}, Best epoch: {stopper.best_epoch}, Counter: {stopper.counter}"
	)
...
def test_arg_defaults():
	...
	parser.add_argument(
		...
		const="last",
		default=None,
		help="recover premature run termination",
	)
...
def refine_stats(
	...
	for split in splits:
		...
		for gloss in stats.keys():
			...
			signers_count += len(
				info["seen_signers"]
			)  # for the average we need to recount
...
def test_shuffle():
	...
	expected = torch.tensor([2.0, 0.0, 4.0, 1.0, 3.0])  # Expected order after shuffle
	assert torch.allclose(shuffled[:, 0, 0, 0], expected), (
		"Custom permutation test failed"
	)
...
def test_sub():
	with subprocess.Popen(
		["ping", "google.com", "-c", "4"],  # Limit to 4 pings
...
def test_safe_index2():
	...
	while len(lst) < 5:
		if abs(i) <= len(lst):
			print(f"{i} in range for len(l) = {len(lst)}")
			print(f"l.insert({i}, {i})")
			lst.insert(i, i)
			print(f"Lst: [{', '.join(map(str, lst))}]")
...
def test_wait_for_completion():
	...
	print(
		gpu_manager.wait_for_completion(
			verbose=True, check_interval=5, confirm_interval=1
		)
	)
...
def shannon_entropy(
	perm: torch.Tensor, unit: Literal["bit", "nat", "dit"] = "bit"
) -> float:
...
def test_loss2():
	...
	print(
		f"loss.item() * batch_size = {loss_mean.item():.6f} * {batch_size} = {loss_mean.item() * batch_size:.6f}"
	)
...
def test_loss3():
	...
	for logits_test, description in test_cases:
		...
		print(f"\n{description}")
		print(
			f"  Probs: cat={probs[0].item():.3f}, dog={probs[1].item():.3f}, bird={probs[2].item():.3f}"
		)
...
def test_raise():
	...
	try:
		...
		print(e)  # This will print just the message string
		# or
