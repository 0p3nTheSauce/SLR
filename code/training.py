...
def save_checkpoint(checkpoint_data: Dict[str, Any], save_path: Path):
	checkpoint = checkpoint_data | {
		...
		"rng_cuda": torch.cuda.get_rng_state_all(),  # list, one per GPU
...
def load_checkpoint(load_path: Path, device: torch.device) -> Dict[str, Any]:
	if load_path.exists():
		...
		if "rng_cuda" in checkpoint:
			...
			raise Warning(f'Checkpoint: {load_path} without rng state was loaded. Reproducability will be impacted.')
		return checkpoint
	else:
		raise ValueError(f"Load path not found: {load_path}")
	
def load_pretrained(check_path: Path, model_name:str, drop_p: float, final_classes: int, extend:bool = True) -> nn.Module:
	...
	og_nc = state_dict["classifier.1.bias"].shape[0]

	if extend:
		model = get_model(model_name, num_classes= og_nc, drop_p=drop_p)
		if og_nc < final_classes:
			model = extend_classifier(model, additional_classes=(og_nc - final_classes))
	else:
...
def train_loop(
	...
	if config["admin"]["weight_path"]:
		model = load_pretrained(Path(config["admin"]["weight_path"]), model_name=model_name, drop_p=drop_p ,final_classes=num_classes)
	else:
