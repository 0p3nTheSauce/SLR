...
class EarlyStopper:
    ...
    def config_precheck(config: Dict[str, Any]) -> Dict[str, Any]:
        """Fail early if early stopping config is invalid. Also set state to off if not present.

        Args:
            config (Dict[str, Any]): Entire training config dictionary.

        Raises:
            ValueError: Missing one of the required keys: 'metric', 'mode', 'patience', 'min_delta'.
            ValueError: Invalid metric specified.
            ValueError: Invalid mode specified.
            ValueError: Patience is not a positive integer.
        ...
        if "early_stopping" not in config["training"]:
            ...
            for key in EarlyStopper.required_keys:
                if key not in es_info:
                    raise ValueError(
                        f"Missing required key: {key} in early stopping config"
                    )

            if es_info["metric"] not in EarlyStopper.available_metrics:
                raise ValueError(
                    f"Invalid metric: {es_info['metric']}. Available metrics: {EarlyStopper.available_metrics}"
                )
            if es_info["mode"] not in EarlyStopper.available_modes:
                raise ValueError(
                    f"Invalid mode: {es_info['mode']}. Available modes: {EarlyStopper.available_modes}"
                )

            if es_info["patience"] <= 0:
                raise ValueError(
                    f"Patience must be a positive integer, got {es_info['patience']}"
                )
            if es_info["min_delta"] < 0:
                raise ValueError(
                    f"Min delta must be non-negative, got {es_info['min_delta']}"
                )
